  ...
  var LunrSearch = (function() {
    ...
    LunrSearch.prototype.compileTemplate = function($template) {
      ...
      return function (view, partials) {
      	var version_regex = /\d\.\d\.\d/i
		var new_view = {}
		new_view.entries=[];
		var current_version = window.location.href.match(version_regex)[0];


		view.entries.forEach(function(entry) {
			var entry_version = entry.url.match(version_regex)[0];
			if (entry_version == current_version){
				new_view.entries.push(entry)
			}
		})
        return Mustache.render(template, new_view, partials);
      };
    };

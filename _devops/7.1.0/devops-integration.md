...
The latest 7.1 Automation API online help is available [here](https://qualisystems.brickftp.com/f/38cd077f7).
...
The 7.1 version of the Sandbox API has some known limitations. It does not support blueprint inputs and the creation of future (pending) reservation of sandboxes. The blueprint inputs feature is available in 8.0. If you have CloudShell 7.1 or below and your CloudShell environment does require these features you may need to fall back to the _cloudshell-automation-api_ package API for sandbox reservation (using the `CreateReservation` method).

### Executing Orchestration Commands

Executing orchestration commands from the Sandbox API is available starting with release 8.0 EA.
Meanwhile, the Automation API package provides the necessary APIs to execute orchestration scripts in the sandbox:
